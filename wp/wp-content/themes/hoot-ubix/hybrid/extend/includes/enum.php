...
function hybridextend_enum_icons( $return = 'list' ) {
	...
	if ( $return == 'sections' || $return == 'section' )
		$list = hybridextend_fonticons_list('sections');

	if ( $return == 'icons' || $return == 'icon' )
		$list = hybridextend_fonticons_list('icons');

	if ( $return == 'lists' || $return == 'list' ) {
		$iconsList = hybridextend_fonticons_list('icons');
		foreach ( $iconsList as $name => $array )
			$list = array_merge( $list, $array );
	}

	return apply_filters( 'hybridextend_enum_icons', $list, $return );
...
function hybridextend_enum_font_sizes( $min = 9, $max = 82 ) {
	static $cache = array();
	$default = wp_parse_args( apply_filters( 'hybridextend_enum_font_sizes', array() ), array(
		'min' => 9,
		'max' => 82,
		) );
	if ( empty( $cache ) )
...
function hybridextend_enum_font_sizes_array( $min = 9, $max = 82, $postfix = 'px' ) {
	$args = wp_parse_args( apply_filters( 'hybridextend_enum_font_sizes', array(
		'min' => $min,
		'max' => $max,
		) ), array(
		'min' => 9,
	...
	$sizes = hybridextend_enum_font_sizes( $args['min'], $args['max'] );
	$output = array();
...
function hybridextend_enum_font_faces( $return = '' ) {
	$fonts = array();
	$webfonts = ( function_exists('hybridextend_fonts_list') ) ? hybridextend_fonts_list() : array();
	$googlefonts = ( function_exists('hybridextend_googlefonts_list') ) ? hybridextend_googlefonts_list() : apply_filters( 'hybridextend_google_fonts', array() );
	...
	return apply_filters( 'hybridextend_enum_font_faces', $fonts, $return );
}
endif;
...
function hybridextend_enum_font_styles() {
	$default = array(
		...
		'semibold'                  => __( 'SemiBold', 'hybrid-core' ),
		'semibold italic'           => __( 'SemiBold Italic', 'hybrid-core' ),
		...
		'uppercase semibold'        => __( 'Uppercase SemiBold', 'hybrid-core' ),
		'uppercase semibold italic' => __( 'Uppercase SemiBold Italic', 'hybrid-core' ),
...
function hybridextend_enum_social_profiles( $skype = true, $email = true ) {
	$social = array(
	...
	return apply_filters( 'hybridextend_enum_social_profiles', $social, $skype );
}
endif;
...
function hybridextend_fonticons_list( $return = 'icons' ) {
	...
	if ( 'sections' == $return ) :
		return apply_filters( 'hybridextend_fonticons_sections', array(
			'fonticons' => __('Font Icons', 'hybrid-core'),
			'brands'    => __('Brands', 'hybrid-core'),
			)
		);
	endif;
 ...
 * @return string
 */
function hybridextend_sanitize_fa( $icon ) {
	$icon = apply_filters( 'hybridextend_migration_icon', trim( $icon ) );
	$migration = array(
