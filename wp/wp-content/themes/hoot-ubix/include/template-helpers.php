...
function hootubix_logo() {
	...
	if ( 'text' == $hootubix_logo || 'custom' == $hootubix_logo ) {
		...
		$title_icon = hybridextend_sanitize_fa( hootubix_get_mod( 'site_title_icon', NULL ) );
...
function hootubix_get_custom_text_logo() {
	...
	return apply_filters( 'hootubix_get_custom_text_logo', $title, $logo_custom );
}
endif;
...
function hootubix_get_logo_image() {
	...
	$class = '';
	$class = ( empty( $class ) ) ? '' : ' class="' . $class . '"';
	$tagline = ( hootubix_get_mod( 'show_tagline' ) ) ? hybrid_get_site_description() : '';
	if ( !empty( $has_logo ) ) {
...
function hootubix_header_aside() {
	...
	?><div <?php hybridextend_attr( 'header-aside', '', "header-aside-{$area}" ); ?>><?php
		...
		if ( $area == 'widget-area' ):
			hybridextend_get_sidebar( 'header' ); // Loads the template-parts/sidebar-header.php template.
		endif;
 ...
 * @return void
 */
if ( !function_exists( 'hootubix_display_loop_title_content' ) ):
function hootubix_display_loop_title_content( $location = 'pre', $context = '' ) {

	$pre_title_content_post = apply_filters( 'hootubix_loop_meta_pre_title_content_post', '', $location, $context );
	if ( ( $location == 'pre' && !$pre_title_content_post ) ||
		 ( $location == 'post' && $pre_title_content_post ) ) : 

		$pre_title_content = apply_filters( 'hootubix_loop_meta_pre_title_content', '', $location, $context );
		if ( !empty( $pre_title_content ) ) :

			$pre_title_content_stretch =  apply_filters( 'hootubix_loop_meta_pre_title_content_stretch', '', $location, $context ); ?>
 ...
 * @return array|bool
 */
if ( !function_exists( 'hootubix_meta_info_display' ) ):
function hootubix_meta_info_display( $args = '', $context = '', $bool = false ) {
...
function hootubix_meta_info_blocks( $args = '', $context = '', $editlink = true ) {

	if ( !is_array( $args ) )
		$args = array_map( 'trim', explode( ',', $args ) );

	$display = hootubix_meta_info_display( $args, $context );
	...
	$blocks = apply_filters( 'hootubix_meta_info_blocks', $blocks, $context, $display, $editlink );
	...
	if ( !empty( $display['publisher'] ) ) {
		...
		echo apply_filters( 'hootubix_entry_byline_publisher', $microdatapublisher );
	}
 ...
 * @return array
 */
if ( !function_exists( 'hootubix_meta_info_blocks_forcedisplay' ) ):
function hootubix_meta_info_blocks_forcedisplay( $display, $context ) {
...
add_filter( 'hootubix_meta_info_blocks_display', 'hootubix_meta_info_blocks_forcedisplay', 5, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'hootubix_display_404_content' ) ):
function hootubix_display_404_content() {
	echo esc_html( __( 'Apologies, but no entries were found.', 'hoot-ubix' ) );
}
endif;
add_action( 'hootubix_404_content', 'hootubix_display_404_content', 5 );
...
function hootubix_post_thumbnail( $classes = '', $size = '', $microdata = false, $link = '', $crop = NULL ) {
 ...
 * @return string
 */
if ( !function_exists( 'hootubix_widget_margin_style' ) ):
function hootubix_widget_margin_style( $mt='', $mb='' ) {
	$return = '';
	if ( $mt===0 || $mt==='0' ) {
 ...
 * @return void
 */
if ( !function_exists( 'hootubix_widgetparam_hook' ) ):
function hootubix_widgetparam_hook() {
	if ( ! is_admin() )
		add_filter( 'dynamic_sidebar_params', 'hootubix_modify_widgetparams' );
}
endif;
add_action( 'wp_loaded', 'hootubix_widgetparam_hook' );
 ...
 * @return array
 */
if ( !function_exists( 'hootubix_modify_widgetparams' ) ):
function hootubix_modify_widgetparams( $params ) {
	global $wp_registered_widgets;
	if ( !isset( $params[0] ) || !isset( $params[0]['widget_id'] ) )
		return $params;
	$widget_id = $params[0]['widget_id']; // Current widget id Eg: hootubix-content-blocks-widget-16
	...
	$widget_opt = get_option( $widget_obj['callback'][0]->option_name ); // all instance values of a particular widget (example: option_name = 'widget_hootubix-content-blocks-widget' )
	...
	if ( !empty( $instance['customcss'] ) ) {
		...
		$string = hootubix_widget_margin_style( $mt, $mb ) . $string;
...
function hootubix_get_lite_slider( $type ) {
	...
	return apply_filters( 'hootubix_get_lite_slider_slides', $slides, $type, $data );
}
endif;
 ...
 * @return void
 */
if ( !function_exists( 'hootubix_ns_filter_avatar' ) ):
function hootubix_ns_filter_avatar( $avatar, $id_or_email, $size, $default, $alt, $args ) {
...
add_filter( 'hootubix_loop_meta_display_title', 'hootubix_hide_loop_meta_static_frontpage' );
 ...
 * @return void
 */
if ( !function_exists( 'hootubix_get_skype_button_code' ) ) :
function hootubix_get_skype_button_code( $username ) {
	static $script = false;
	static $id = 1;
	$code = '';
	$action = apply_filters( 'hootubix_skype_button_action', 'call' );
	...
	$code .= '<div id="SkypeButton_Call_' . $username . '_' . $id . '" class="hootubix-skype-call-button">';
	$code .= '<script type="text/javascript">';
	$code .=  'Skype.ui({'
	...
	$code = apply_filters( 'hootubix_get_skype_button_code', $code, $script, $id, $action );
	$script = true;
	$id++;
	return $code;
}
