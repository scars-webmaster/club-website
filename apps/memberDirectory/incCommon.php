	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'memberInfo' => array('MemberInfo', '', 'table.gif', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'memberInfo' => "`memberInfo`.`id` as 'id', `memberInfo`.`fname` as 'fname', `memberInfo`.`lname` as 'lname', `memberInfo`.`suffix` as 'suffix', `memberInfo`.`middle` as 'middle', `memberInfo`.`memberDate` as 'memberDate', `memberInfo`.`homePhone` as 'homePhone', `memberInfo`.`workPhone` as 'workPhone', `memberInfo`.`mobilePhone` as 'mobilePhone', `memberInfo`.`emailPreferred` as 'emailPreferred', `memberInfo`.`emailOther` as 'emailOther', `memberInfo`.`addressLine1` as 'addressLine1', `memberInfo`.`addressLine2` as 'addressLine2', `memberInfo`.`city` as 'city', `memberInfo`.`state` as 'state', `memberInfo`.`zip` as 'zip'"
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('SCARS_Membership_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('SCARS_Membership_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['SCARS_Membership_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['SCARS_Membership_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> SCARS Membership</a>
			</div>
